...
export const fetchOrders = createAsyncThunk("orders/fetchOrders", async () => {
  const response = await axios.get("https://makeupbackend2-0.onrender.com/api/orders", {
    headers: {
...
export const fetchAdminOrders = createAsyncThunk(
  ...
  async () => {
    const response = await axios.get(
      "https://makeupbackend2-0.onrender.com/api/orders/admin/orders",
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
...
export const updateOrderStatus = createAsyncThunk(
  ...
  async ({ orderId, status }: { orderId: number; status: string }) => {
    const response = await axios.put(
      `https://makeupbackend2-0.onrender.com/api/orders/admin/orders/${orderId}`,
      { status },
      {
        headers: {
          "Content-Type": "application/json",
...
export const fetchOrderById = createAsyncThunk(
  ...
  async (orderId: number) => {
    const response = await axios.get(
      `https://makeupbackend2-0.onrender.com/api/orders/${orderId}`,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
